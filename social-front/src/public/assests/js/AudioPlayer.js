...
AudioPlayer.prototype._drawLoadingAnimation = function () {
  ...
  for (var i = 0; i < 8; i++) {
    ...
    this._drawingContext.arc(
      this.width / 2 - i * 20 + 60,
...
AudioPlayer.prototype._loadAudioFile = function () {
  ...
  request.onload = function () {
    if (request.status == 200) {
      self._audioContext.decodeAudioData(
        request.response,
        function (buffer) {
          ...
          for (var i = 0; i < self._audioData.length; i++) {
            if (i % self.scaleFactor == 0) {
              ...
              self._sampledData[i / self.scaleFactor] = Math.min(
                Math.max(scaledSample, -self.ceiling),
                self.ceiling
              );
              avgSample = 0;
            }
